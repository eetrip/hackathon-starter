    ...
    if (req.user) {
      User.findOne({ twitch: profile.id }, (err, existingUser) => {
        ...
        if (existingUser && existingUser.id !== req.user.id) {
          req.flash("errors", {
            msg:
              "There is already a Twitch account that belongs to you. Sign in with that account or delete it, then link it with your current account.",
          });
          done(err);
        } else {
          User.findById(req.user.id, (err, user) => {
      ...
      User.findOne({ twitch: profile.id }, (err, existingUser) => {
        ...
        User.findOne({ email: profile.email }, (err, existingEmailUser) => {
          ...
          if (existingEmailUser) {
            req.flash("errors", {
              msg:
                "There is already an account using this email address. Sign in to that account and link it with Twtich manually from Account Settings.",
            });
            done(err);
          } else {
            const user = new User();
